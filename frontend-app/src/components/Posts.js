...
class Posts extends Component {
  ...
  renderPagination() {
    ...
    const prev = () => {
      if (current_page > 1) {
        return (<Link to={`/dashboard/posts?page=${pagination.current_page - 1}`} className="pagination-previous is-hidden-mobile">Prev</Link>);
      } else {
        return (<a className="pagination-previous is-hidden-mobile" disabled="disabled">Prev</a>);
      }
    };

    const next = () => {
      if (current_page < last_page) {
        return (<Link to={`/dashboard/posts?page=${pagination.current_page + 1}`} className="pagination-next is-hidden-mobile">Next</Link>);
      } else {
        return (<a className="pagination-next is-hidden-mobile" disabled="disabled">Next</a>);
      }
    };
